...
const Image = ({ data }) => {
    return (
        <div class="relative group overflow-hidden rounded-lg shadow-lg shadow-shdw hover:opacity-90 transition duration-300">
            <a href={data.links.html} title={"View " + data.user.username + "'s photo on Unsplash"} target="_blank" rel="noreferrer">
                ...
                <a href={data.user.links.html} title={"View " + data.user.username + "'s works on Unsplash"} target="_blank" rel="noreferrer">
                    <figcaption class="absolute flex justify-center items-center text-white opacity-0 bottom-0 inset-x-0 pt-8 pb-4 bg-gradient-to-t from-black transition duration-300 group-hover:opacity-100 hover:from-gray-dk hover:text-teal-lt">{"@" + data.user.username}</figcaption>
                </a>
            </a>
        </div>
    );
